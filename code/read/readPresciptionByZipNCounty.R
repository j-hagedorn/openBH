...
# create a function to combine data recorded by zipcode
combine_all_by_zip <- function(){
  # load older data
  ...
  combined_by_zip <- rbind(combined_by_zip, combined_invalid_zip, combined_out_of_state)
  
  return(combined_by_zip)
}

# create a function to combine data recorded by county
combine_all_by_county <- function(){
  # load newer data
  ...
  combined_by_county <- rbind(
    ...
    rename(
      SUBJECT = id,
      DRUG_NAME_AND_STRENGTH = DRUG.NAME.STRENGTH,
      DEA_DRUG_SCHEDULE = DEA.DRUG.SCHEDULE,
      AHFS_DESCRIPTION = AHFS.DESCRIPTION,
      SCRIPTS = PRESCRIPTION.COUNT....,
      UNITS = PRESCRIPTION.QUANTITY....
    )
  
  return(combined_by_county)
}

# load the data and store as dataframes
combined_by_zip_df <- combine_all_by_zip()
combined_by_county <- combine_all_by_county()
